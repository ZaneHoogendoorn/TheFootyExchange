...
# Generate AFL Player List
def Price_Calculator(First_Last, link):
    playerdetails = [] 
    ...
    playerdetails.append(First_Last)
    playerdetails.append(round(c_purchase_price, 2))
    playerdetails.append(c_R1)
    playerdetails.append(round(c_sale_price, 2))
    playerdetails.append(round(lastr,2))
    playerdetails.append(round(last5,2))
    playerdetails.append(round(last10,2))
    playerdetails.append(Valuation)
    
    return playerdetails

def Update_Players_and_Prices():
    ...
    new_links = []
    for link in links:
        if link is not None and len(link) >= 7 and link[:7] == "players":
    ...
    df1[0] = df1[0].str.strip('[')
    df1[28] = df1[0].str.strip(']')

    df6 = df1.dropna(axis=0, how='any')

    surnames = list(df6[1])
    firstnames = list(df6[2])

    df7 = []
    for i in range(len(surnames)):
        name = firstnames[i].strip(' ') + "_" + surnames[i].strip(' ')
        df7.append(name)
        
    df8 = df7

    name_and_link = []
    for b in range(len(df8)):
        name_and_link.append([df8[b], new_links[b]])
    ...
    for i in range(len(name_and_link)):
        First_Last = name_and_link[i][0]
        link = name_and_link[i][1]
        try:
            player_info = Price_Calculator(First_Last, link)
    ...
    prices_table[0] = prices_table[0].str.strip('0')
    prices_table[0] = prices_table[0].str.strip('1')
    prices_table[0] = prices_table[0].str.strip('_')
    prices_table.columns = ['Name','Buy','Price','Sell','Last Rd %','L5 %', 'L10 %', "Valuation"]

    todaysdate = datetime.today().strftime('%Y-%m-%d')

    prices_table.to_csv('Players_and_Prices.csv')

def Update_Names_and_Link():
    url = "https://afltables.com/afl/stats/2021.html"
    html = urlopen(url)
    ...
    names_and_links = pd.DataFrame.from_records(name_and_link)

    names_and_links.columns = ['Name','Link']

    names_and_links.to_csv('Names_and_link.csv')
